...
def filter_pts(x, lines, line_types, eps=0.):
    mask = torch.full((len(x),), True, dtype=torch.bool).cuda()
...
def line_indicator(a, b, c, t=1e5, eps=0., line_type='line'):
    if line_type == 'line':
        ...
        center = torch.tensor([a, b]).cuda()
        return lambda x: torch.sigmoid(-t * (((x - center) ** 2).sum(dim=1) - (c ** 2 + eps)))


def shape_indicator_fn(lines, line_types, eps=1e-4):
    ...
    return lambda x: 200 * prod([indicator(x) for indicator in indicators1]) * (1 - prod([indicator(x) for indicator in indicators2]))
...
class SDF(nn.Module):
    ...
    def get_set(self, N=256, margin=1.5):
        ...
        pts = get_grid(margin * self.a, int_margin).cuda()
        S = self(pts)
        if self. shape_function is not None:
    ...
    def repulsion_loss(self, epsilon=0.001):
        ...
        epsilon = torch.full(dist.shape, epsilon * self.a).cuda()
...
for frequency, basis_init, lambdas_init, epsilon_init, A_weight, sigmoid_t, entropy_weight, repulsion_weight, repulsion_epsilon, weight_decay, boundary_weight in combinations:
    ...
    print('shape function', shape_function)
    shape_function=[[[0, 0, 1.2 * a]], ['circle']]
    # shape_function = [[[0, 0, 1.1 * a]], ['circle']]
    ...
    iters = 1000
    optimizer = optim.Adam(sdf.parameters(), lr=1e-5)
    total_loss = 0.
    lr_change = {100: 1e-6}
